'''
CHAMPS Dataset preprocessing Class Implementation
(atom_index_0, type) tuple version.
...
class CHAMPSData:
    ...
    def make_X(self, dfTrain1):
        ...
        dfTrain2 = pd.get_dummies(dfTrain1[['molecule_name', 'atom_index_1', 'ai0_type']], columns=['ai0_type'])

        print('CREATING THE FEATURE MATRIX')
        if self.dfXtest is None:
            dfTrain2 = dfTrain2.groupby(['molecule_name', 'atom_index_1']).sum()
            ...
            dfTrain3 = dfTrain2.xs('train').groupby(['molecule_name', 'atom_index_1']).sum()
            ...
            dfTrain3 = dfTrain2.xs('test').groupby(['molecule_name', 'atom_index_1']).sum()
    ...
    def make_Y(self, dfTrain1):
        ...
        dfTrain2 = pd.get_dummies(
            dfTrain1[['id', 'molecule_name', 'atom_index_1', 'scalar_coupling_constant', 'ai0_type']],
            columns=['ai0_type'])
        dfTrain2.iloc[:, 4:] = dfTrain2.iloc[:, 4:].multiply(dfTrain2.iloc[:, 3], axis='index')

        if self.dfXtest is None:
            dfTrain3 = dfTrain2.iloc[:, np.r_[1, 2, 4:dfTrain2.shape[1]]].groupby(['molecule_name', 'atom_index_1']).sum()
            ...
            dfTrain3 = dfTrain2.xs('train').iloc[:, np.r_[1, 2, 4:dfTrain2.shape[1]]].groupby(['molecule_name', 'atom_index_1']).sum()
            ...
            dfTrain3 = dfTrain2.xs('test').iloc[:, np.r_[1, 2, 4:dfTrain2.shape[1]]].groupby(['molecule_name', 'atom_index_1']).sum()
            self.Y_dict_tst = self.fn_matrix(dfTrain3, self.F, self.N,
                                         isflatten=True)
    ...
    def make_YID(self, dfTrain1):
        ...
        dfTrain2 = pd.get_dummies(
            dfTrain1[['id', 'molecule_name', 'atom_index_1', 'ai0_type']], columns=['ai0_type'])
        ...
        if self.dfXtest is None:
            dfTrain3 = dfTrain2.iloc[:, np.r_[1, 2, 3:dfTrain2.shape[1]]].groupby(
                ['molecule_name', 'atom_index_1']).sum()
            ...
            dfTrain3 = dfTrain2.xs('train').iloc[:, np.r_[1, 2, 3:dfTrain2.shape[1]]].groupby(
                ['molecule_name', 'atom_index_1']).sum()
            self.YID_dict_trn = self.fn_matrix(dfTrain3, self.F, self.N, isflatten=True)
    ...
    def form_feat_matrix(self):
        ...
        dfTrain1['ai0_type'] = list(zip(dfTrain1['atom_index_0'], dfTrain1['type']))

        dfTrain1['ai0_type'] = dfTrain1['ai0_type'].apply(lambda x: str(x[0]) + '_' + x[1])

        self.F = dfTrain1['ai0_type'].nunique()
        print('N is', self.N)
        print('F is', self.F)
    ...
    def prep_champs_data(self, gen_file=True):
        ...
        '''
        if gen_file:
            print('Preparing feature matrix, scalar constant vector and ID vector...')
            self.form_feat_matrix()

            print('Preparing adjacency matrix')
            self.make_adjm()

        print('ALL DONE, NOW COMBINING DICTS')
